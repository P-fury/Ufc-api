...
def parse_sherdog_fighter(url):
    ...
    fighter = {
        'name': xml.xpath("//span[@class='fn']/text()")[0] if xml.xpath("//span[@class='fn']/text()") else None,
        'nickname': bio.xpath("//span[@class='nickname']/em/text()")[0] if bio.xpath(
            "//span[@class='nickname']/em/text()") else 'NaN',
        'nationality': bio.xpath("//strong[@itemprop='nationality']/text()")[0] if bio.xpath(
            "//strong[@itemprop='nationality']/text()") else 'NaN',
        'birthplace': xml.xpath("//span[@class='locality']/text()")[0] if xml.xpath(
            "//span[@class='locality']/text()") else 'NaN',
        'birthdate': xml.xpath("//span[@itemprop='birthDate']/text()")[0] if xml.xpath(
            "//span[@itemprop='birthDate']/text()") else 'NaN',
        'age': xml.xpath("//span[@itemprop='birthDate']/preceding-sibling::b/text()")[0] if xml.xpath(
            "//span[@itemprop='birthDate']/preceding-sibling::b/text()") else 'NaN',
        'height': xml.xpath("//b[@itemprop='height']/text()")[0] if xml.xpath(
            "//b[@itemprop='height']/text()") else 'NaN',
        'weight': xml.xpath("//b[@itemprop='weight']/text()")[0] if xml.xpath(
            "//b[@itemprop='weight']/text()") else 'NaN',
        'association': xml.xpath("//span[@itemprop='memberOf']/a/span/text()")[0] if xml.xpath(
            "//span[@itemprop='memberOf']/a/span/text()") else 'NaN',
        'weight_class': xml.xpath("//div[@class='association-class']/a/text()")[0] if xml.xpath(
            "//div[@class='association-class']/a/text()") else 'NaN',
...
def get_ufc_link(query):
    ...
    url = f"https://www.ufc.com/athlete/{query_formatted}"
    return url
...
def get_fighter(query):
    try:
        ...
        csv_filename = f"{fighter['name'].lower()}.csv"

        with open(csv_filename, mode='w', newline='') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=fighter.keys())
            writer.writeheader()
...
def get_upcoming_event_links():
    ...
    htm = req.get(url, headers=headers)
    xml = html.document_fromstring(htm.content)
    return ["https://www.ufc.com/" + x for x in xml.xpath(
        "//details[@id='events-list-upcoming']/div/div/div/div/div/section/ul/li/article/div[1]/div/a/@href")]
